...
int parallel() {
    ...
    double Input[NUMPAT+1][NUMIN+1] = { 0, 0, 0,  0, 1, 1,  0, 1, 0,  0, 0, 1,  0, 0, 0 };
    double Target[NUMPAT+1][NUMOUT+1] = { 0, 0,  0, 1,  0, 0,  0, 0,  0, 1 };
...
#pragma omp for schedule reduction (+: DeltaWeightIH, WeightIH )
...
#pragma omp for schedule reduction (+: DeltaWeightHO, WeightHO )
...
#pragma omp for schedule reduction (+: SumDOW, DeltaH )
            for( j = 1 ; j <= NumHidden ; j++ ) {    /* 'back-propagate' errors to hidden layer */
                SumDOW[j] = 0.0 ;
#pragma omp for schedule reduction (+: SumDOW)
